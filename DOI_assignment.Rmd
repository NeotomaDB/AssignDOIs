...
`RelatedIdentifiers` should be DOIs.  This needs to be implemented.

`Size` needs to be implemented.
...
So, the schema doesn't validate properly, but:
...
where `r paste0('data/datasets/', dataset, '_output.xml')` then represents the new, modified, XML document for the record.