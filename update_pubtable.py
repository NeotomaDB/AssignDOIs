...
with open('publications.json', 'w') as f:
	...
	for rows in doi_test:
		if rows[1] == "Yes":
			i= i + 1
			print("What's up?")
			doi = rows[7]
			r = requests.get(urlbase + doi)
			
			doi_dict = json.loads(r.text).get("message")

			# Now add the publication ID from the Publications Table:
			doi_dict['PublicationID'] = rows[3]

			json.dump(doi_dict, f)
